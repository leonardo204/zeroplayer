...
마지막 갱신: 2026-09-25 (2.0 빌드 2 를 심사에 제출했다 — WAITING_FOR_REVIEW)
...
**2.0 이 심사 대기 중이다.** 2026-09-24 에 빌드 2 로 제출했고 상태는 `WAITING_FOR_REVIEW` 다.
빌드 1 은 `ITMS-91064`(개인정보 매니페스트의 추적 선언)로 거부됐다 — 6-16-3-1 참고.
지금 코드를 고치면 다음 버전(2.0.1)으로 나간다. 심사 중에 빌드를 갈아 끼우려면
제출을 먼저 취소해야 한다.
...
`ZPFakeNowPlaying 1` 은 재생 없이 미니 플레이어만 띄운다. 네트워크를 타지 않고 화면
배치만 볼 때 쓴다. `ZPPauseResume 1` 은 재생 5초 뒤 멈추고 3초 뒤 다시 재생한다.
...
**시뮬레이터에서 앱이 조용히 죽던 것은 오디오가 아니라 기기 안 모델이었다.**
한동안 맥 오디오 문제로 적어 뒀는데 틀렸다. 크래시 리포트 세 건이 전부
`FoundationModels` 였다(6-14 참고). 그쪽을 막은 뒤로 시뮬레이터에서도 재생이 잘 된다.

### 두 번째 회차 (2026-09-24)

실기기 로그에 남던 경고 두 가지를 고쳤다. 둘 다 동작을 막지는 않았지만 실제 비용이 있었다.

**잠금화면 상태를 두 방식으로 알려 주고 있었다.** `MPNowPlayingInfoCenter.playbackState` 는
iOS 에서 `com.apple.mediaremote.set-playback-state` 엔타이틀먼트가 있는 앱만 쓴다. 없으면
시스템이 무시하면서 `[MRNowPlaying] Ignoring setPlaybackState` 를 남긴다. 재생 여부는
`MPNowPlayingInfoPropertyPlaybackRate` 로 이미 전달되므로 그 줄을 지웠다.

**오디오 세션을 메인 스레드에서 켜고 있었다.** `setCategory`·`setActive` 는 수십 밀리초를
잡아먹어서 AVFoundation 이 "UI unresponsiveness" 를 경고한다. 방송을 누른 순간 화면이
멈칫하는 원인이다. `AudioSessionManager.offMainThread` 로 옮기고 결과만 기다린다.

이 때문에 `activate()` 와 `AudioPlayerService.resume()` 이 `async` 가 됐다. 화면 단추와
원격 명령·인터럽트 복귀는 `Task { await player.resume() }` 로 부른다. **`await` 가 하나
늘었으므로 그 사이에 다른 항목으로 넘어갔는지 다시 확인한다** — `play()` 와 `resume()` 에
guard 를 한 줄씩 더 뒀다. 세션을 내리는 `deactivate()` 는 아무도 기다릴 필요가 없어 뒤에서
도는데, 그대로 두면 방금 켠 세션을 뒤늦게 끄는 일이 생긴다(프리셋 자동 선택이 죽은 방송을
만나 다음 후보로 넘어갈 때가 그 순서다). 그래서 `pending` 으로 부른 순서를 지킨다.

확인은 `-ZPPauseResume 1` 로 한다. 재생 5초 뒤 멈추고 3초 뒤 다시 재생한다.
고친 뒤 같은 로그에서 두 경고 모두 0건이다.
...
**실기기에서 확인할 것** — 재생 중에 바뀌는 그림이라 실제로 들어 봐야 안다.
...
앱 화면과 **서버가 만드는 문구**를 한국어·영어 둘로 낸다. 방송국 이름과 한국 지상파
편성표는 옮기지 않는다 — 고유명사라 바꾸면 스토어·검색에서 쓰는 이름과 갈라진다.

**앱 화면은 문자열 카탈로그 하나가 맡는다.** `Sources/Resources/Localizable.xcstrings`
이고 **열쇠가 한국어 원문 그대로**다. 원본 언어가 한국어라서, 한국어는 열쇠를 그대로
쓰고 영어만 번역을 담는다. 문구를 새로 넣을 때는 한국어로 쓰고 이 파일에 영어 한 줄을
더하면 된다. 영어를 빠뜨리면 그 자리만 한국어로 나온다(앱이 깨지지는 않는다).

| 자리 | 어떻게 되나 |
| --- | --- |
| `Text("취침")` 처럼 SwiftUI 에 바로 넣은 글자 | 그대로 둔다. 카탈로그가 알아서 바꾼다 |
| `String` 을 돌려주는 곳(모델·오류 문구·알림 본문) | `String(localized: "…")` 로 감싼다 |
| `log.info("…")` · LLM 프롬프트 · `#if DEBUG` 안 | 감싸지 않는다. 개발자만 본다 |

보간이 든 문구는 열쇠가 자리표시자 모양이 된다 — `"\(minutes)분"` 의 열쇠는 `%lld분`,
...
**추천 세트가 두 배가 된다.** 세트 id 에 언어가 붙어 `상황|평일주말|시간대|나라|언어` 이고
조합이 120 → 240 이다. 매일 새벽 배치는 그대로 24개씩 채우므로 열흘이면 다 찬다. 세트가
없는 언어는 규칙 문구로 즉석에서 만들어 주니 배치가 밀려도 목록은 나온다.

**언어는 앱 안에서 바꾸지 않는다.** 설정의 '언어' 줄은 iOS 설정 앱의 이 앱 화면을 연다.
다국어 번들이면 iOS 가 거기에 언어 항목을 만들어 주고, 고르면 앱을 다시 열어 주며 바로
반영한다. 앱 안에서 바꾸려면 `Bundle.main` 의 문자열 조회를 가로채야 하는데
(`object_setClass` 로 `localizedString(forKey:)` 를 덮는 방법) **SwiftUI 의 `Text` 에는
통하지 않는다** — 실제로 만들어 확인했고 화면이 그대로 한국어였다. `.environment(\.locale)`
만 바꾸는 것도 통하지 않는다. 지금 언어는 `AppLanguage.code` 로 읽는다. 기기 설정이 아니라
**번들이 실제로 고른 값**(`Bundle.main.preferredLocalizations`)이라 화면과 늘 맞고,
서버에 보내는 `lang` 도 이 값을 쓴다.

## 6-16-1. 화면 밝기

설정의 '화면 밝기' 에서 기기 설정 따름·밝게·어둡게를 고른다. `AppTheme` 이고 값은
`@AppStorage("zp.theme")` 에 남는다. `RootView` 가 `preferredColorScheme` 으로 걸어서
재생 화면 같은 시트까지 함께 따라간다.

## 6-16-2. 취소를 실패로 세지 않는다

화면을 옮기거나 검색어를 다시 넣으면 앞선 요청이 취소된다. 이걸 실패로 보면 두 곳이 탈난다 —
목록이 캐시로 내려앉아 옛 목록이 새 목록을 덮고, Apple Intelligence 가 두 번 만에 꺼진다.
실기기 로그에 찍힌 `네트워크 실패로 캐시 50건을 쓴다` 와 `CancellationError()` 가 이것이었다.
`ProxyError.cancelled` 를 두고 `isCancellation(_:)` 으로 가린다. 취소일 때는 오류 문구를
띄우지 않고 캐시로 내려가지도 않는다. 네트워크 실패를 다루는 코드를 새로 쓸 때는 이 구분을
먼저 넣는다.

## 6-16-3. 제출 전 점검에서 나온 것

릴리스 설정으로 빌드해 산출물을 열어 보고 두 구멍을 메웠다.

**개인정보 매니페스트가 없었다.** `Sources/Resources/PrivacyInfo.xcprivacy` 를 만들어
번들 루트에 들어가는 것까지 확인했다. 없으면 업로드할 때 `ITMS-91053` 로 막힌다 —
`@AppStorage`(테마·타이머 기본값)와 재생 속도 기억에 UserDefaults 를 쓰는데 사유를
안 밝혔기 때문이다. 사유는 `CA92.1` 이다. AdMob·UMP 는 자기 매니페스트를 프레임워크
안에 들고 오므로 앱이 따로 적지 않는다. 사유를 밝혀야 하는 API 를 새로 쓰면
(파일 타임스탬프 `C617.1`·디스크 여유 `E174.1`·부팅 시각 `35F9.1`·활성 키보드 `54BD.1`)
이 파일에 한 줄 더한다.

**추적 허가 문구가 한국어뿐이었다.** `Info.plist` 의 `NSUserTrackingUsageDescription`
은 한 언어만 담는다. 영어 기기에서도 한국어 문구가 떴다. `ko/en` 의
...
제출에 들어간 것 — 빌드 2, 스크린샷 아이폰 6장·아이패드 6장(한국어만),
설명·키워드·프로모션·릴리스 노트, 심사 메모. 스토어 현지화는 **한국어 하나**다.

**승인된 뒤에 할 일**

- [ ] AdMob 앱 인증을 다시 누른다. 마케팅 URL 이 스토어에 반영돼야 `app-ads.txt` 를 찾는다
- [ ] 빠진 스크린샷 넉 장을 채운다 — 아이폰 `03-timer`·`08-alarm`, 아이패드 `01-recommend`·`03-timer`.
      애플 쪽 처리가 막혀 지웠던 것이다(6-16-7). 다음 버전에서 `tools/asc-upload-screenshots.py` 로 올린다
- [ ] 영어 현지화를 더할지 정한다. 더하면 `docs/09-appstore-submit.md` 의 영어 문구와
      `Screenshots/jpg-en`·`jpg-ipad-en` 을 함께 올린다

**반려되면** 사유 코드를 먼저 확인한다. 이메일 제목은
"App Store Connect: Your app binary has been rejected" 이고 `ITMS-9xxxx` 코드가 본문에 있다.
지금까지 걸린 셋은 6-16-3-1 에 원문과 함께 적어 뒀다.

**아직 안 한 콘솔 작업**

- [ ] **내 기기를 AdMob 테스트 기기로 등록한다** — 가장 급하다. 배포 빌드가 실제 광고를
      받으므로 내가 내 광고를 누르면 무효 트래픽이고, 계정 정지는 앱이 아니라 계정 단위다.
...
- [x] 기기 안 모델이 실기기에서는 실제로 문구를 만든다 (2026-09-24 로그로 확인)
- [x] 알림 권한을 허용하면 APNs 토큰이 잡히고 서버에 등록된다 (`pushableDevices: 1` 로 확인)
...
- [ ] 설정의 '화면 밝기' 를 밝게·어둡게로 바꿔 모든 탭과 재생 화면이 따라가는지
- [ ] 설정의 '언어' 를 눌러 iOS 설정 화면이 열리고, 거기서 English 를 고르면 앱이 다시 열리는지
- [ ] 기기 언어를 영어로 두고 앱 화면·추천 문구·알람 알림이 영어로 나오는지
- [ ] App Store Connect 에 영어 스토어 설명을 함께 올린다

**실기기에서 이미 확인된 것** (2026-09-24)

- 1.7 자료 이관 — 즐겨찾기 11개가 넘어왔다. 히든·알람·유튜브는 원래 안 쓰던 값이라 `false`
- 배너가 네 화면에 실제로 뜬다 (`배너 받음` 다섯 번)
- 만료 서명 주소를 걷어낸 뒤 KBS Classic FM 이 `radio.bsod.kr` 주소로 재생된다
...
- AdMob 앱 등록이 앱 인증에서 막혀 있다. 원인과 푸는 법은 6-18 절. 실제 광고 단위 ID 는
